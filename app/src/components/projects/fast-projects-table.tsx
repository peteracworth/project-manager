...
interface FastProjectsTableProps {
  projects: Project[];
  onUpdate?: (projectId: string, field: string, value: any) => Promise<void>;
}

export function FastProjectsTable({ projects, onUpdate }: FastProjectsTableProps) {
  ...
  const columns = useMemo<ColumnDef<Project>[]>(
    () => [
      {
        ...
        cell: ({ getValue, row }) => (
          <EditableCell
            ...
            onChange={(newValue) => onUpdate?.(row.original.id, "title", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <SelectCell
            ...
            onChange={(newValue) => onUpdate?.(row.original.id, "status", newValue)}
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <SelectCell
            ...
            onChange={(newValue) => onUpdate?.(row.original.id, "priority", newValue)}
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <SelectCell
            ...
            onChange={(newValue) => onUpdate?.(row.original.id, "task_progress", newValue)}
      ...
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <EditableCell
            value={getValue() as string}
            onChange={(newValue) => onUpdate?.(row.original.id, "location", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <EditableCell
            value={getValue() as string}
            onChange={(newValue) => onUpdate?.(row.original.id, "project_area", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <EditableCell
            value={getValue() as string}
            onChange={(newValue) => onUpdate?.(row.original.id, "description", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <MultiSelectCell
            value={(getValue() as string[]) || []}
            onChange={(newValue) => onUpdate?.(row.original.id, "tags", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <DateCell
            value={getValue() as string}
            onChange={(newValue) => onUpdate?.(row.original.id, "start_date", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => (
          <DateCell
            value={getValue() as string}
            onChange={(newValue) => onUpdate?.(row.original.id, "due_date", newValue)}
          />
        ),
      },
      {
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <ProjectReferenceCell
              ...
              onChange={(newIds) => onUpdate?.(row.original.id, "blocked_by", newIds)}
            />
          );
        },
      },
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <ProjectReferenceCell
              ...
              allProjects={data}
              currentProjectId={row.original.id}
              onChange={(newIds) => onUpdate?.(row.original.id, "blocking", newIds)}
            />
          );
        },
      },
    ...
    [allUsers, onUpdate, selectOptions, data, handleRefresh]
  );
  ...
  return (
    <div className="space-y-4" style={{ userSelect: isAnyColumnResizing ? 'none' : undefined }}>
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="relative border-r border-b border-gray-200 px-3 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider"
      ...
      <div className="text-sm text-gray-500">
        {table.getRowModel().rows.length} project(s)
      </div>
    </div>
  );
}
